...
class Penjualan_pesanan extends CI_Controller {
	...
	public function cetak($param)
	{
		...
		{				  		 
			...
			$border = array('','',);
			$align  = array('C','C');
			$pdf->FancyRow(array('Disetujui',''),$fc,$border, $align);
			$pdf->ln(1);
			$pdf->ln(15);
	...
	public function edit($nomor)
	{
		...
		{	
			...
			$detil  = $this->db->select('ar_sodetail.*, inv_barang.namabarang')->
			join('inv_barang','inv_barang.kodeitem=ar_sodetail.kodeitem')->get_where('ar_sodetail', array('kodeso' => $nomor));
